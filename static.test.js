...
assert.ok(html.includes("Version 3.6.0"), "index version label is stale");
...
for (const phrase of ['APP_VERSION = "3.6.0"', 'basis: "gross"', "investmentPct: 20", "schg: 20"]) {
  assert.ok(core.includes(phrase), `${phrase} not found in core.js`);
}

const sw = fs.readFileSync(path.join(root, "service-worker.js"), "utf8");
assert.ok(sw.includes("driver-command-3.6.0-v1"), "service-worker cache version is stale");
...
assert.ok(readme.includes("# Driver Command 3.6.0"));
